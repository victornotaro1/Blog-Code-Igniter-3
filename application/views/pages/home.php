...
<?php foreach (array_slice($posts, 0, 1) as $index => $post) : ?>
    <div class="jumbotron p-3 p-md-5 text-white rounded bg-success" 
        ...
        <div class="col-md-6 px-0">
            ...
            <button type="button" class="btn" data-toggle="modal" data-target="#modal-jumbotron">
                <a href="#" class="text-light">Continue lendo</a>
            </button>
        </div>
    </div> 

    <div class="modal fade" id="modal-jumbotron" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="TituloModalCentralizado"><?= $post["titulo"] ?></h5>
...
<div class="row mb-2">
    <?php foreach (array_slice($posts, 1, 2) as $index => $post) : ?>
        <div class="col-md-6">
            <div class="card h-100" style="background-image: url('<?= $post['image_url'] ?>'); background-size: cover; background-position: center; height: 300px;">
                <div class="card-body d-flex flex-column align-items-start" style="background-color: rgba(0, 0, 0, 0.5);">
                    ...
                    <button type="button" class="btn" data-toggle="modal" data-target="#modal-card-<?= $index ?>">
                        <a href="#" class="text-light">Continue lendo</a>
                    </button>
                </div>
            </div>
        ...
        <div class="modal fade" id="modal-card-<?= $index ?>" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado-<?= $index ?>" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
            ...
            <?php foreach (array_slice($posts, 3, 3) as $index => $post) : ?>
                <div class="blog-post">
                    ...
                    <div class="text-right">
                        <button type="button" class="btn" data-toggle="modal" data-target="#modal-main-<?= $index ?>">
                            Continuar Lendo...
                        </button>
                    </div>
                    <hr>
                ...
                <div class="modal fade" id="modal-main-<?= $index ?>" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado-<?= $index ?>" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="TituloModalCentralizado-<?= $index ?>"><?= $post["titulo"] ?></h5>
